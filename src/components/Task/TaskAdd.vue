    ...
    <div id="index">
        <div class="breadcrumb">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item><span class="color-blue">新增任务</span></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
...
import store from '../../vuex/store.js';
    export default{
        ...
        methods:{
            ...
            selects(id,index){
                ...
                this.$api.task.getMakeTempType(params).then(res=> {
                    if(res.data.status == 200){
                        ...
                        if(data){
                            if(!this.form.contentList[index].taRemark){
                                this.form.contentList[index].taRemark = data.taskRequirement;
                            }
                            if(!this.form.contentList[index].tempFileId){
            ...
            submitDatas(data){
                console.log(data);
                this.$api.task.taskPostEdit(data).then(res=> {
                    if(res.data.status == 200){
        ...
        beforeMount(){
            ...
            if(this.aid != 0 ){
                //获取修改的企业信息
                let params = {taId:this.aid}
                this.$api.list.taskAddInfo(params).then(res=> {
                    console.log(res);
                    if(res.data.status == 200){    
                        let that = this;                    
                        let arr = res.data.data.task.contentList;
                        ...
                        arr.forEach(item=>{
                            ...
                            objs.conCount = item.conCount;
                            objs.conPrice = item.conPrice;
                            objs.valueTime[0] = item.beginTime;
                            objs.valueTime[1] = item.endTime;
                            objs.taRemark = item.taRemark;
                            objs.prodId = item.prodId;
                            objs.tempFileId = item.tempFileId;
                            if(item.tempFileId){
                        ...
                        that.form.taId = res.data.data.task.taId;
                        that.form.taName = res.data.data.task.taName;
                        that.form.capitalId = res.data.data.task.capitalId;
                        that.form.cityId = res.data.data.task.cityId;
                        let parmasCity = {capitalId : res.data.data.task.capitalId};
                        this.$api.list.getCity(parmasCity).then(res=> {
                            if(res.data.status == 200){
                                this.city = res.data.data.list;
                            }else{
                                this.$message.error(res.data.msg);
