            ...
            <el-row :gutter="20">
                ...
                <el-col :span="8">
                    <el-form ref="form" :model="form" label-width="120px" class="information-content padding20">
                        ...
                        <el-form-item label="转载站名：" v-if="form.newsCome == 1">
                            <el-input v-model="form.source" placeholder="请输入转载站名"></el-input>
                        </el-form-item>
...
            },
      ...
      console.log(this.editorOption)
        ...
        methods:{
            ...
            back(){
                this.$router.push({path:'/ContentMat'})
            },
            //上传
            ...
            submit(){
                if(this.form.newsCome == 2){ //判断为原创修改source字段为原创
                    this.form.source = "原创";
                }
                ...
                this.$api.list.getToken().then(res=> {
                    if(res.data.status == 200){
                        ...
                        if(id == 0){
                            console.log(params);
                            this.$api.list.insertCategoryContent(params).then(res=> {
                                if(res.data.status == 200){
                            ...
                            this.$api.list.editCategoryContent(params).then(res=> {
                                if(res.data.status == 200){
                                    this.$message.success('操作成功！');
                                    this.$router.push({
                                        path:'/ContentMat'
        ...
        beforeMount(){
            ...
            if(this.aid != 0 ){
                ...
                this.$api.list.getCategoryContentInfo(params).then(res=> {
                    if(res.data.status == 200){
                        this.form = res.data.data.tbNews;
